...
if (PinAnswer.pin === mypin) {console.log("correct pin number")
...
let enteramount = Remainingamount.amount
   { if (enteramount >= mybalance) {console.log("You have insufficient balance")}
    
   else (mybalance -= Remainingamount.amount)
  ...
  if (easyCash >= mybalance) {console.log("You have insufficient balance")}

  else (mybalance -= easyCash)
    {console.log(`Your remaining balance is : ${mybalance}`)}
}
    else (operationPin.operation === "balance check") ;{console.log(`Your balance is ${mybalance}`)
    }
